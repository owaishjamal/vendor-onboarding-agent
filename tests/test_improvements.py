...
# ===========================================================================
# Reviewer-override report
# ===========================================================================

def test_override_report_flags_a_disagreement():
    # Reviewer approves a case the system had held → an override on the
    # consistency/bank-name check.
    c = run("VS-03_kessler_bank_mismatch.json")
    cases.record_action(c["case_id"], "approve", reviewer="tohid",
                        note="Confirmed account holder by phone")
    rep = cases.override_report()
    assert rep["override_count"] == 1
    assert "consistency" in rep["by_check"]


def test_override_report_ignores_agreement():
    # Reviewer rejects a case the system already flagged for review — that's
    # agreement in direction (both non-approve), not an override to approve.
    c = run("VS-04_sundara_country_mismatch.json")
    cases.record_action(c["case_id"], "reject", reviewer="tohid", note="")
    rep = cases.override_report()
    # rejecting a PENDING_REVIEW is not an "approved despite flag" override
    assert all(o["direction"] != "approved despite flag" for o in rep["overrides"])
...
def test_volume_eval_holds_at_scale():
    """Regression guard: precision and recall stay perfect on generated volume."""
    sys.path.insert(0, str(ROOT / "scripts"))
    from backend.app.checks import registry as reg
    import importlib
    ev = importlib.import_module("eval_volume")

    cases_gen, registry = ev.generate(220)
    reg.set_registry_override(registry)
    try:
        auto_fp = 0
        fraud_total = fraud_caught = 0
        for sub, cat in cases_gen:
            status, _, _ = __import__(
                "backend.app.pipeline.runner", fromlist=["assess"]
            ).assess(VendorSubmission(**sub))
            got = status.value
            want = ev.EXPECTED[cat]
            if got == "APPROVED" and want != "APPROVED":
                auto_fp += 1
            if cat in ev.FRAUD_CATEGORIES:
                fraud_total += 1
                fraud_caught += got in ("PENDING_REVIEW", "REJECTED")
    finally:
        reg.set_registry_override(None)

    assert auto_fp == 0, f"{auto_fp} fraudulent vendors were auto-approved"
    assert fraud_caught == fraud_total, "a fraud signal was missed at volume"
